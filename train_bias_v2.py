...
# Better lean tradeoff than 128x2
MAX_LENGTH = 128
MAX_CHUNKS = 3
...
# More lean-friendly defaults
W_LEAN_HARD = 1.5
W_LEAN_SOFT = 0.5
W_INTENSITY = 1.0
W_DOMAIN = 0.0   # test with 0.0 first; reintroduce later only if useful
...
class HierTextCollator:
    """
    For pseudo-label inference only.
    Returns row_id + Batch
    """
...
def map_text_to_model_inputs(tokenizer, df: pd.DataFrame) -> HFDataset:
    ...
    def map_fn(batch):
        ...
        out["lean_soft"] = batch["lean_soft"] if "lean_soft" in batch else [None] * len(texts)
        return out
...
def train_teacher_headlines(seed: int, encoder_path: str, use_compile: bool = False) -> str:
    ...
    train_df, val_df = train_test_split(
        df_h, test_size=0.08, random_state=42, stratify=df_h["y_lean"]
    )
...
def _read_pseudo_cache() -> pd.DataFrame:
    ...
    if PSEUDO_PARTS_DIR.exists():
        parts = sorted(PSEUDO_PARTS_DIR.glob("part_*.parquet"))
        for p in parts:
            try:
    ...
    # deduplicate by row_id, keep latest
...
def pseudo_label_articles(teacher_dir: str, encoder_path: str, infer_batch: int = INFER_BATCH, use_compile: bool = False):
    ...
    for item in tqdm(dl, desc="pseudo-label", dynamic_ncols=True):
        ...
        with torch.autocast(device_type=DEVICE, enabled=USE_AMP, dtype=AMP_DTYPE):
            out = model(batch, grl_lambda=0.0)
            probs = (
                torch.softmax(out["logits_lean"], dim=-1)
                .float()
                .detach()
                .cpu()
                .numpy()
    ...
    print("[Pseudo-label] done. Cache saved.")


# ============================================================
# STUDENT DATAFRAME BUILD
# ============================================================
def build_student_dataframe_only():
    """
    Builds memory-safe train/val dataframes with:
    - separate downsampling by task label
    - separate train/val splits by task label
    """
    df_h, df_a = load_and_prepare_raw()
    pseudo_df = _read_pseudo_cache()

    # merge pseudo probs to articles
    df_a = df_a.merge(pseudo_df, on="row_id", how="left")

    # convert p0..p4 to lean_soft only if teacher confident enough
    def make_soft(row):
        if pd.isna(row["p0"]):
            return None
        probs = [float(row["p0"]), float(row["p1"]), float(row["p2"]), float(row["p3"]), float(row["p4"])]
        if max(probs) < PSEUDO_MIN_CONF:
            return None
        return probs
    ...
    # --------------------------------------------------------
    if STUDENT_DATA_FRACTION < 1.0:
        print(f"[Student] Using {int(STUDENT_DATA_FRACTION * 100)}% of dataset")

        df_h = (
            df_h.groupby("y_lean", group_keys=False)
            .apply(lambda x: x.sample(frac=STUDENT_DATA_FRACTION, random_state=42))
            .reset_index(drop=True)
        )

        df_a = (
            df_a.groupby("y_int", group_keys=False)
            .apply(lambda x: x.sample(frac=STUDENT_DATA_FRACTION, random_state=42))
            .reset_index(drop=True)
        )

    # --------------------------------------------------------
    # separate stratified split
    # --------------------------------------------------------
    h_train, h_val = train_test_split(
        df_h,
        test_size=0.05,
        random_state=42,
        stratify=df_h["y_lean"]
    )
    ...
    train_df = pd.concat([h_train, a_train], ignore_index=True)
    val_df = pd.concat([h_val, a_val], ignore_index=True)
...
def maybe_take_training_break(start_time: float, save_callback=None):
    ...
    if elapsed_hours >= BREAK_EVERY_HOURS:
        ...
        if save_callback is not None:
            print("Saving checkpoint before break...")
            save_callback()
...
def train_student(seed: int, encoder_path: str, run_name: str, use_compile: bool = False) -> str:
    ...
    # Optional warm-start: freeze encoder for epoch 1
    freeze_first_epoch = True

    for epoch in range(1, EPOCHS_STUDENT + 1):
        if freeze_first_epoch and epoch == 1:
        ...
        for batch_idx, batch in pbar:
            training_start_time = maybe_take_training_break(
                training_start_time,
...
def predict(text: str, model_dir: str, encoder_path: str, threshold_non_center: float = 0.5):
    ...
    probs_int = (
        torch.softmax(out["logits_int"], dim=-1)
        .float()
        .squeeze(0)
        .cpu()
        .numpy()
...
def main():
    ...
    if args.train_teacher:
        teacher_dir = train_teacher_headlines(
            seed=args.seed,
